...
def extract_chapter(assignment, assignment_number, narrative_number):
    text = assignment.read_text(encoding="utf-8")
    marker = re.compile(
        rf"^## Assignment {assignment_number}(?: / Episode \d+)?\s+—.*$",
        re.MULTILINE,
    )
    next_marker = re.compile(r"^## Assignment \d+", re.MULTILINE)
    match = marker.search(text)
    if match is None:
        raise ValueError(f"Assignment {assignment_number} is missing from {assignment}")
    chapter = text[match.end() :]
    next_match = next_marker.search(chapter)
...
def main():
    ...
    assignment = (root / args.assignment).resolve()
...
def compose_assignment(
    ...
    assignment, scenes = extract_chapter(
        assignment, assignment_number, narrative_number
    )
    episode_number = assignment_number - 100
    ...
    layouts = []
    output_folder = root / "feelingverse/source" / narrative_id / episode_folder
